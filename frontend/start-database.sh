...
# Cleanly parse DATABASE_URL (strip query parameters like ?sslmode=...)
DB_URL_NO_QUERY="${DATABASE_URL%%\?*}"

DB_PASSWORD=$(echo "$DB_URL_NO_QUERY" | awk -F':' '{print $3}' | awk -F'@' '{print $1}')
DB_PORT=$(echo "$DB_URL_NO_QUERY" | awk -F':' '{print $4}' | awk -F'/' '{print $1}')
if [ -z "$DB_PORT" ]; then
  DB_PORT="5432"
fi
